...
--
-- Host: 127.0.0.1
-- Generation Time: Maj 31, 2024 at 12:37 AM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetActivatedTestAnswers` (IN `p_link_account_activated_test_id` INT)   BEGIN
    SELECT 
        link_account_activated_tests_answer.id, 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAnswerDetails` (IN `p_link_account_activated_tests_answer_id` INT)   BEGIN
    SELECT 
        answers.text AS answer_text 
    FROM 
        answers 
        LEFT JOIN questions ON answers.question_id = questions.id 
        LEFT JOIN link_account_activated_tests_answer ON link_account_activated_tests_answer.question_id = questions.id 
    WHERE 
        link_account_activated_tests_answer.id = p_link_account_activated_tests_answer_id;
END$$
...
(8, '2024-06-01 10:00:00', 'JKL123', 4, 2, 1),
(9, '2024-06-01 11:00:00', 'MNO456', 5, 8, 1),
(10, '2024-06-01 12:00:00', 'PQR789', 6, 11, 1);
...
CREATE TRIGGER `after_insert_link_account_activated_tests_answer` AFTER INSERT ON `link_account_activated_tests_answer` FOR EACH ROW BEGIN
    ...
    IF (SELECT opened FROM questions WHERE id = NEW.question_id) = 0 THEN
        ...
        -- Jeśli wszystkie odpowiedzi są poprawne, dodaj punkty
        IF total_answers = correct_answers THEN
            UPDATE link_account_activated_tests_answer
