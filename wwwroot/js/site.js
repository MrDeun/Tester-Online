...
function addGroupRow(table_id, input_id){
    ...
    if(input.value != ""){
        ...
        id.innerHTML = "";
        name.innerHTML = "<input type='hidden' name='groups[]' id='group_" + row.sectionRowIndex + "' value='" + input.value + "'>" + input.value;
...
function addAnswerRow(table_id, input_id){
    ...
    if(input.value != ""){
        ...
        if(row.sectionRowIndex <= 10){
            ...
            var deleteRow = row.insertCell();
            id.innerHTML = "";
            text.innerHTML = "<input type='hidden' name='answers[]' id='answer_" + row.sectionRowIndex + "' value='" + input.value + "'>" + input.value;
            correct.innerHTML = "<td><input type='checkbox' class='w3-check' name='answers_correct[]' value='new_" + row.sectionRowIndex + "'></td>";
            deleteRow.innerHTML = "<button type='button' class='w3-button w3-red' onclick='deleteTableRow(this)'>Usuń</button>";
        }
    }
